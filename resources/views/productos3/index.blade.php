...
<center>
<center><h1> MERCOSUR - PRODUCTOS_BRASIL</h1></center>
<center><button><a href="{{url('/consulta')}}">REGRESAR AL LISTADO DE PAISES</a></button></center><br>
    <div id="form">
        <form method="post" action="{{ route('productos3.store') }}">
        {{ csrf_field() }}
        ...
        <table widht="100%" border="1" cellpadding="2" align="center">
            ...
            <tr>
                ...
                <td>{{$producto3->tipo}}</td>
                <td>{{$producto3->precio}}</td>
                <td>{{$producto3->cantidad}}</td>
                <td>{{$producto3->descripcion}}</td>
                <form action="{{route('productos3.destroy',$producto3->id)}}" method="POST">
                @csrf
                @method('DELETE')
