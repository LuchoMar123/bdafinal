...
<center>
<center><h1> MERCOSUR - PRODUCTOS_PARAGUAY</h1></center>
<center><button><a href="{{url('/consulta')}}">REGRESAR AL LISTADO DE PAISES</a></button></center><br>
    <div id="form">
        <form method="post" action="{{ route('productos4.store') }}">
        {{ csrf_field() }}
        ...
        <table widht="100%" border="1" cellpadding="2" align="center">
            ...
            <tr>
                <td>{{$producto4->id}}</td>
                <td>{{$producto4->nombre}}</td>
                <td>{{$producto4->tipo}}</td>
                <td>{{$producto4->precio}}</td>
                <td>{{$producto4->cantidad}}</td>
                <td>{{$producto4->descripcion}}</td>
                <form action="{{route('productos4.destroy',$producto4->id)}}" method="POST">
                @csrf
                @method('DELETE')
