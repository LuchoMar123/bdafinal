...
<center>
<center><h1> MERCOSUR - PRODUCTOS_VENEZUELA</h1></center>
<center><button><a href="{{url('/consulta')}}">REGRESAR AL LISTADO DE PAISES</a></button></center><br>
    <div id="form">
        <form method="post" action="{{ route('productos6.store') }}">
        {{ csrf_field() }}
        ...
        <table widht="100%" border="1" cellpadding="2" align="center">
            ...
            @foreach($productos6 as $producto6)
            <tr>
                <td>{{$producto6->id}}</td>
                <td>{{$producto6->nombre}}</td>
                <td>{{$producto6->tipo}}</td>
                <td>{{$producto6->precio}}</td>
                <td>{{$producto6->cantidad}}</td>
                <td>{{$producto6->descripcion}}</td>
                <form action="{{route('productos6.destroy',$producto6->id)}}" method="POST">
                @csrf
                @method('DELETE')
