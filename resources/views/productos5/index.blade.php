...
<center>
<center><h1> MERCOSUR - PRODUCTOS_URUGUAY</h1></center>
<center><button><a href="{{url('/consulta')}}">REGRESAR AL LISTADO DE PAISES</a></button></center><br>
    <div id="form">
        <form method="post" action="{{ route('productos5.store') }}">
        {{ csrf_field() }}
        ...
        <table widht="100%" border="1" cellpadding="2" align="center">
            ...
            @foreach($productos5 as $producto5)
            <tr>
                <td>{{$producto5->id}}</td>
                <td>{{$producto5->nombre}}</td>
                <td>{{$producto5->tipo}}</td>
                <td>{{$producto5->precio}}</td>
                <td>{{$producto5->cantidad}}</td>
                <td>{{$producto5->descripcion}}</td>
                <form action="{{route('productos5.destroy',$producto5->id)}}" method="POST">
                @csrf
                @method('DELETE')
